...
$$i = \mathrm{argmin}_{i \in \{1 \dots 50\}} \mathrm{RMSE(lm(}\cos_1 + \dots + \cos_i + \sin_1 + \dots + \sin_i))$$
...
knitr::kable(int_gam_res, caption = "Comparing GAM models with and without interaction for the weighted wind. Equation are defined in the attached R code snippet in the text.", col.names = c("Equation", "GCV score", "Residual Deviance"), align = c("l", "r", "r"))
```
...
Ensuite, au lieu de modéliser le quantile $0.95$ par :
```{r eval = FALSE}
m <- mean(gam$residuals),
s <- sd(gam$residuals)
quant <- qnorm(0.95, mean = m, sd = s)
```
nous avons également utilisé une regréssion quantile linéaire sur les résidus :
```{r eval = FALSE}
mod.rq <- rq(
...
```
Cependant, avec l'inclusion de certaines variables, la matrice de design peut devenir singulière. Dans cet esprit, nous avons donc également testé une modélisation directe du quantile 0.95 à l'aide de modèle GAM-quantile. 

### Résultats des GAM

Pour conclure sur cette section, nous rapportons ici les résultats du meilleur modèle GAM obtenu lors du data challenge :
```{r}
